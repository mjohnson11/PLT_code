...
# input file: should have columns with headers that say "Diverse_BC", "Environment_BC", and "Total_Counts"
bc_csv = output_dir + run_name + '_bc_counts.csv'
# cluster output file
out_csv = output_dir + run_name + '_clusters.csv'
# output base for cluster info
full_out_base = output_dir + run_name
...
class ClusterParser:
    ...
    for Diverse_BC, Environment_BC, Total_Counts
    """
    ...
    def __init__(self, f_in, bc_in):
        ...
        self.total_counts_col = list(self.bcdata['Total_Counts'])
    ...
    def output_clusters(self, f_out):
        ...
        total_reads_index = self.cols_to_add.index('Total_Counts')
        total_reads_clust = 0
        total_bc_clust = 0
    ...
    def output_unclustered(self, f_out):
        ...
        unclust_reads = sum(unclust_data['Total_Counts'])
...
def div_bc_clustering(ec, dat):
    div_bcs_list = dat.as_matrix(['Diverse_BC', 'Total_Counts'])
    ...
    full_bc_list = dat.as_matrix(['Diverse_BC', 'Environment_BC', 'Total_Counts'])
    ...
    bc_out = full_out_base + '_bc_counts_clustered.csv'
    centroids_only_out = full_out_base + '_centroids_only.csv'
    cluster_details = full_out_base + '_cluster_info.csv'
    unclustered_out = full_out_base + '_unclustered.csv'
    ...
    with open(full_out_base + '_error_correct_stats.csv', 'w') as outfile:
        writer = csv.writer(outfile)
        writer.writerow(['Run', 'Raw_BCs', 'Raw_Reads', 'Centroids', 'Clustered_BCs', 'Clustered_Reads',
                         'Unclustered_BCs', 'Unclustered_Reads', 'Centroids_Excluded_On_Thresh',
                         'Reads_Excluded_On_Thresh'])
        writer.writerow([run_name, len(dat), sum(dat['Total_Counts']), len(my_clus_pars.centroids_dict),
...
env_bcs_list = bcdata.as_matrix(['Environment_BC', 'Total_Counts'])
